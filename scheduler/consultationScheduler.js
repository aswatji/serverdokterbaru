...
class ConsultationScheduler {
  ...
  start() {
    ...
    this.intervals.set('expiry-check', setInterval(() => {
      this.checkExpiringConsultations();
    }, 60000)); // 60 seconds

    // Check for consultations expiring in 5 minutes (warning)
    this.intervals.set('expiry-warning', setInterval(() => {
  ...
  async checkExpiringConsultations() {
    try {
      ...
      if (expiredConsultations.length > 0) {
        console.log(`Found ${expiredConsultations.length} expired consultations`);
        ...
        console.log(`Deactivated ${expiredConsultations.length} expired consultations`);
      }
    } catch (error) {
  ...
  async checkConsultationsExpiringSoon() {
    try {
      ...
      if (expiringSoonConsultations.length > 0) {
        console.log(`Found ${expiringSoonConsultations.length} consultations expiring soon`);

        // Notify via Socket.IO
        if (global.socketServer) {
          expiringSoonConsultations.forEach(consultation => {
            const timeRemaining = Math.ceil((consultation.expiresAt.getTime() - now.getTime()) / 60000);
