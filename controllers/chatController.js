...
class ChatController {
  ...
  // =======================================================
  // 🔹 CREATE (or GET) CHAT - ✅ SOLUSI ERROR 500
  // =======================================================
  async createChat(req, res) {
    try {
      const { userId, doctorId } = req.body;
      ...
      // 1. CARI: Cek apakah chat sudah ada
      let chat = await prisma.chat.findFirst({
        where: { userId, doctorId },
        include: {
          user: { select: { id: true, fullname: true, photo: true } },
      ...
      if (!chat) {
        console.log(
          `🆕 Membuat chat baru untuk User:${userId} & Doc:${doctorId}`
        );
        const chatKey = `${userId}-${doctorId}`;
        ...
        try {
          ...
          // Jika gagal create (misal karena tabrakan request dalam milidetik yang sama),
          // maka kita cari lagi datanya (karena berarti sudah dibuat oleh request paralel lain)
          console.warn("⚠️ Chat creation collision, fetching existing one...");
      ...
      // Jika masih null (sangat jarang terjadi), return error sopan
      if (!chat) {
        return res
          .status(500)
          .json({ success: false, message: "Gagal inisialisasi chat" });
      }

      // 3. CEK PEMBAYARAN AKTIF (Safe Logic)
      const now = new Date();
      ...
      // 4. RETURN SUKSES (200)
      // Selalu kembalikan 200 OK agar frontend tidak looping error
      const responseData = {
        ...chat,
  ...
  async sendMessage(req, res) {
    try {
      ...
      const unreadWhere =
        senderType === "user"
          ? { chatId_doctorId: { chatId: chat.id, doctorId: targetId } }
          : { chatId_userId: { chatId: chat.id, userId: targetId } };
      ...
      prisma.chatUnread
        .upsert({
          ...
          create: {
            chatId: chat.id,
            userId: senderType === "doctor" ? targetId : null,
            doctorId: senderType === "user" ? targetId : null,
            unreadCount: 1,
          },
        })
        .catch((e) => console.warn("Unread failed", e));
      ...
      setImmediate(async () => {
        try {
          ...
          console.warn("Socket/Notif Err", e.message);
        }
      });
  ...
  async sendFileMessage(req, res) {
    try {
      ...
      setImmediate(() => {
        ...
        // Logic notif sama seperti sendMessage...
      });

      return res.status(201).json({ success: true, data: message });
    } catch (error) {
      console.error("❌ Error sendFile:", error);
      res.status(500).json({ success: false, message: error.message });
    }
  }
  ...
  async deleteMessage(req, res) {
    try {
      ...
      const message = await prisma.chatMessage.findUnique({
        where: { id: messageId },
        include: { chatDate: { include: { chat: true } } },
      });
      ...
      if (message.type === "image" || message.type === "pdf") {
        const parts = message.content.split(`/${bucketName}/`);
        if (parts[1])
          await deleteFromMinio(parts[1]).catch((e) => console.warn(e));
      }

      await prisma.chatMessage.delete({ where: { id: messageId } });

      const io = getIO();
      io.to(`chat:${message.chatDate.chat.id}`).emit("message_deleted", {
        messageId,
        chatId: message.chatDate.chat.id,
      });
  ...
  async editMessage(req, res) {
    try {
      ...
      const updated = await prisma.chatMessage.update({
        ...
        data: { content },
        include: {
          replyTo: { select: { id: true, content: true, sender: true } },
        },
      });

      const io = getIO();
      io.to(`chat:${message.chatDate.chat.id}`).emit("message_updated", {
        chatId: message.chatDate.chat.id,
        message: updated,
      });
  ...
  async extendSession(req, res) {
    try {
      ...
      if (!lastPayment)
        return res.status(400).json({ message: "No active payment" });
      ...
      return res
        .status(200)
        .json({ success: true, data: { expiresAt: newExpiresAt } });
    } catch (error) {
      console.error("❌ Error extendSession:", error);
      res.status(500).json({ success: false, message: error.message });
    }
