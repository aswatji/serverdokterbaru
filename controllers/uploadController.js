...
class UploadController {
  ...
  static async uploadFile(req, res) {
    try {
      ...
      try {
        if (sender === chat.userId) {
          ...
          try {
            ...
            /* ignore socket error for unread */
          }
      ...
      // ============================================================
      // 🔥 BROADCAST KE SOCKET (BAGIAN PALING PENTING)
      // ============================================================
      try {
        // 1. Ambil instance socket yang SAMA dengan chatSocket.js
        const io = getIO();

        if (io) {
          // 2. Gunakan nama room dengan prefix "chat:"
          const roomName = `chat:${chat.id}`;

          // 3. Cek apakah ada penghuni (Optional Debugging)
          const room = io.sockets.adapter.rooms.get(roomName);
          const memberCount = room ? room.size : 0;

          console.log(
            `📢 [UploadController] Broadcasting to room: ${roomName}`
          );
  ...
  static async uploadMultiple(req, res) {
    try {
      ...
      // Loop upload logic (simplified for brevity, logic sama dengan single)
      // ... (Bagian validasi & loop file Anda sebelumnya sudah benar)
      // ...

      // Saat broadcast di dalam loop uploadMultiple:
      /*
         const io = getIO(); // Gunakan getIO()
         if (io) {
            io.to(`chat:${chat.id}`).emit("new_message", messagePayload); // ✅ PASTIKAN ADA 'chat:'
         }
      */

      // Note: Untuk uploadMultiple, pastikan Anda juga menerapkan
      // import { getIO } dan prefix `chat:` seperti di uploadFile.

      // ... (Kode return response) ...

      // Return placeholder (sesuaikan dengan logika full Anda jika perlu)
      return res.status(200).json({
        success: true,
        message: "Batch upload processed (Check logs)",
      });
    } catch (err) {
